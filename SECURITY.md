...
- Every Agent SDK child environment removes `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_TOKEN`, `ANTHROPIC_BASE_URL`, `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`, and all `ANTHROPIC_VERTEX_*` variables. The prompt is parked behind a gate and is only dispatched after SDK account metadata confirms a first-party Claude subscription — a rejected account never receives user content. The bridge additionally rejects explicit or missing API-key credential sources and aborts when SDK rate-limit metadata reports `isUsingOverage: true`. Claude Code currently reports `apiKeySource: "none"` for valid Keychain-backed subscription OAuth, so account metadata is the primary check.
